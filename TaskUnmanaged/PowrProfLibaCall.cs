...
Applications can use this level to retrieve information about the idleness of the system.
  ...
  UCHAR CoolingMode;
        ...
        UInt64 IPowrProfLibaCall.LastSleepTime()
        {
            UInt64 LastSleepTime;
            var status = CallNtPowerInformation(15, IntPtr.Zero, 0, out LastSleepTime, Marshal.SizeOf(typeof(UInt64)));
            if (status == 0)
            {
        ...
        UInt64 IPowrProfLibaCall.LastWakeTime()
        {
            UInt64 LastWakeTime;

            var status = CallNtPowerInformation(14, IntPtr.Zero, 0, out LastWakeTime, Marshal.SizeOf(typeof(UInt64)));
            if (status == 0)
            {
                return LastWakeTime;
            }
        ...
        UInt64 IPowrProfLibaCall.SystemReserveHiberFile(bool Restore)
        {
            UInt64 SystemReserveHiberFile;
            var status = CallNtPowerInformation(10, Restore, Marshal.SizeOf(typeof(Boolean)), out SystemReserveHiberFile, Marshal.SizeOf(typeof(UInt64)));
            if (status == 0)
            {
